...
function calcPrice(cost) {
    ...
    // cover it!
    const postage = 10;
    return Math.ceil((cost * 2.5) + postage);
}
...
export default function calcPrintSize(...args) {
    ...
    return availablePrintSizes.filter(available => {
        ...
        let matchScore = ((longestEdge / shortestEdge) - (available.x / available.y)) * 10;
        matchScore < 0 ? matchScore = matchScore * -1 : matchScore;
        return matchScore < 1 && longestEdge > available.x && shortestEdge > available.y
